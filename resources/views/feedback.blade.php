    ...
    <div class="max-w-6xl mx-auto space-y-8">
        ...
        <div class="space-y-6">
            @forelse($feedbacks as $feedback)
                <div
                    class="feedback-card p-8 shadow-sm opacity-0 transform translate-y-4 transition-all duration-700 feedback-item">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-12 h-12 rounded-lg bg-indigo-50 flex items-center justify-center border border-indigo-100 shadow-sm overflow-hidden">
                                @if ($feedback->user && $feedback->user->profile)
                                    <img src="{{ asset('storage/' . $feedback->user->profile) }}"
                                        alt="{{ $feedback->user->name }}" class="w-full h-full object-cover">
                                @else
                            ...
                            <div>
                                <h5 class="text-lg font-bold text-slate-800">{{ $feedback->user->name ?? 'Guest User' }}
                                </h5>
                        ...
                        <div
                            class="inline-flex items-center px-3 py-1 bg-amber-50 border border-amber-100 rounded-lg  font-bold text-md">
                    ...
                    @if ($feedback->room)
                        <div
                            class="inline-flex items-center px-3 py-1.5 bg-slate-50 text-slate-600 rounded-md text-md font-semibold mb-4">
                    ...
                    @if ($feedback->admin_reply)
                        <div class="admin-reply">
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <span class="text-md font-bold text-indigo-900 uppercase tracking-tighter">Management
                                    Response</span>
                            </div>
